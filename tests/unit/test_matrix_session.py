...
class _LateKeyCrypto:
    """Decrypt returns None (UTD) until the key 'arrives' (has_key flips True)."""
    ...
    async def decrypt(self, ev, room_id):
        if not self.has_key:
            return None
        return {"type": "m.room.message", "content": {"msgtype": "m.text", "body": "hi"}}


async def test_utd_buffered_then_recovered_when_key_arrives() -> None:
    creds = BotCreds("@plugin:hs", "DEV", "tok", "wss://gw/ws")
    cap: list = []

    async def on_msg(r, sender, c, eid=""):
        cap.append((r, sender, eid))

    s = MatrixSession(creds, on_user_message=on_msg)
    s.gateway = RequestGateway()
    crypto = _LateKeyCrypto()
    s.crypto = crypto
    s.rooms = FakeRooms()

    # Message arrives before its key → UTD → buffered, NOT dropped, not dispatched.
    ev = {"type": "m.room.encrypted", "sender": "@u:hs", "event_id": "$m1",
          "content": {"session_id": "X"}}
    await s._handle_encrypted("!sess:hs", ev)
    assert cap == []
    assert len(s._utd_buffer) == 1

    # Next sync imports the key → retry recovers + dispatches the buffered message.
    crypto.has_key = True
    await s._on_sync({"pos": "p2"})
    assert cap == [("!sess:hs", "@u:hs", "$m1")]
    assert s._utd_buffer == []
